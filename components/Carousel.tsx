...
export default function Carousel({
    ...
    const scrollView:
        MutableRefObject<ScrollViewComponent> | any =
    ...
    useEffect(() => {
        ...
        if (handleIndexChange) handleIndexChange(
            { item: data[selectedIndex], index: selectedIndex })
    }, [selectedIndex]);
    ...
    const setIndex =
        (event: NativeSyntheticEvent<NativeScrollEvent>):
            void => {
            const contentOffset:
                NativeScrollPoint = event.nativeEvent.contentOffset;
            const viewSize:
                NativeScrollSize = event.nativeEvent.layoutMeasurement;
